...
class TTSApp(ctk.CTk):
    def __init__(self):
        ...
        # Center Window
        window_width = 500
        window_height = 600  # Increased height slightly for new dropdown
    ...
    def _setup_ui(self):
        ...
        voice_names = self.audio.get_voice_names()  # <--- NEW
        ...
        ctk.CTkLabel(self, text="Voice:").pack(pady=(10, 5))
        self.voice_dropdown = ctk.CTkOptionMenu(
            self, values=voice_names, width=350)
        self.voice_dropdown.pack(pady=5)

        # Set default to first voice if available (usually David or Zira)
        if voice_names:
            self.voice_dropdown.set(voice_names[1])

        # 3. Secondary Speaker Checkbox
    ...
    def _process_speech_thread(self, mode):
        ...
        try:
            # <--- CHANGED: Pass the selected voice name here
            selected_voice = self.voice_dropdown.get()
            self.audio.generate_tts_file(
                text, self.speed_slider.get(), selected_voice)
